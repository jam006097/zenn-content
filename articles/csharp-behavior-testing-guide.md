...
実装の詳細をテストすると、コード의 内部構造（アルゴリズムの改善や変数名の変更など）を変えるたびにテストコードも修正が必要になります。これを**「脆いテスト（Brittle Tests）」**と呼びます。
...
今日から、privateメソッドのテストをやめて、そのクラスが提供する「価値（振る舞い）」をテストしてみましょう。
