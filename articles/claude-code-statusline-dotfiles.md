...
Claude Code を使い込んでいくうちに、こんな不安が出てきました。

- レートリミットをどのくらい使っているのか、作業中に把握できない
- Extra Usage（従量課金）を有効にしているため、気づかないうちに課金が積み上がっていないか心配
- コンテキストをどのくらい消費しているかわからず、応答がおかしくなるまで気づけない

都度ブラウザでダッシュボードを確認するのは面倒で、作業の流れが途切れます。そこで Claude Code のステータスバーに使用量をリアルタイム表示するようにしました。
